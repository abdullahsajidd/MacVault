...
export const catalogModelType = defineType({
  ...
  fields: [
    ...
    defineField({name: 'sourceName', title: 'Specification source', type: 'string', readOnly: true}),
    defineField({name: 'sourceUrl', title: 'Source URL', type: 'url', readOnly: true, validation: (rule) => rule.required()}),
    defineField({name: 'sourceUpdatedAt', title: 'Source imported at', type: 'datetime', readOnly: true}),
